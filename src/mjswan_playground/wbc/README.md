...
config**. This task supplies only what that config does not carry: the trained policy and
the clips, which live in the deploy repo, cloned into `.cache/`; set
`MJSWAN_WBC_DEPLOY_ROOT` to use a checkout you already have.
...
reference slots on `TrackingCommand`. With those, wbc's own observation functions trace
unmodified (no second copy of the math here) and `mjswan.compile.run_parity` checks
every term against the live `Wbc-G1` env.
