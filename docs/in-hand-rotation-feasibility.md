...
# 続報：ポリシーが正しく動かなかった原因（2026-08-21 追記2）

最初のデモはビルドも実行も通っていましたが、**方策としては機能していませんでした**。
原因は2つあり、どちらも mjswan ではなくこのプロトタイプ側の問題でした。

## 原因1：reset イベントを空にしていた

`events={}` としていました。mjlab はエンティティの `init_state` を**リセットイベント経由で**
適用するので、イベントが無いと初期姿勢が一切書かれません。実測したところ、手は全関節ゼロ、
キューブはワールド原点に置かれていました（`root_link_pos_w = [0,0,0]`、1ステップで落下判定）。

上流の3つの reset イベント（`reset_base` / `reset_robot_joints` / `reset_cube_pose`）を
ランダム化レンジをゼロにして復元しました。

関連して、映像には出ないもう1つのバグ：手の `init_state.joint_pos` に grasp cache が記録した
「落ち着いた後の手の姿勢」を使っていました。上流の cache リセットは**キューブの寸法と姿勢しか
書きません**（"Robot joints are not touched"）。手は常に固定の `GRASP_INIT_JOINT_POS` から始まり、
それが `joint_pos_rel` の引く基準でもあります。cache の姿勢を使ったことで、方策の320入力のうち
160が系統的にずれていました。

## 原因2：mjlab の相対行動項では回らない

これが本題です。上流は**指令値**に積分（`q_cmd += δ`、ソフト関節限界でクランプ）、
mjlab の `RelativeJointPositionAction` は**測定値**基準（`q_cmd = q + δ`）。
接触下では測定値が指令値に遅れるため、積分型が保持できる目標を測定値基準型は到達できません。

`rollout_check.py` で実際の mjlab env に ONNX 方策を 400 ステップ（20秒）流して比較しました:

| 行動項 | キューブ yaw | レート | 落下 |
|---|---|---|---|
| 指令値積分（上流） | **−6.10 rad**（0.97回転） | **−0.305 rad/s** | なし |
| 測定値基準（mjlab） | 0.02 rad | 0.001 rad/s | なし |

上流の学習目標は 0.20 rad/s なので、積分型はそれを上回り、mjlab の項は**まったく回りません**。
「mjlab にあるものだけ取り込む」方針では、この方策は動かないということです。

## 追加した機能とその位置づけ

mjswan の `RelativeJointPositionActionCfg` に `relative_to` を追加しました
（`"position"` = mjlab 互換・既定、`"command"` = 積分型）。**これは mjlab パリティではありません** ──
mjlab は測定値基準しか持っていません。ただし指令値積分は HORA / LEAPSim 系の標準的な
デルタ制御であり、タスク固有の発明ではないので、同一クラスのフラグとして実装し、
既定は mjlab の意味論のままにしてあります。

付随して必要になったもの:

- **一制御ステップにつき1回だけ積分する**フック（`beginControlStep`）。mjswan の `applyAction` は
  substep ごとに呼ばれるので、そこで積分すると decimation 倍だけ進んでしまう。mjlab の
  `process_actions` / `apply_actions` の分割に対応する
- **ソフト関節限界へのクランプ**。ブラウザは自前でモデルをコンパイルするので、`jnt_range` は
  持っている。運ぶ必要があるのは `soft_joint_pos_limit_factor` の float 1つだけ
- **リセット時に現在の関節角から積分器を再初期化**（mjlab の `_initialize_target` に対応）
- **decimation 内の setpoint 線形補間**（上流の `interpolate_decimation`）

TS テストは 373 件パス（新規 11 件）。うち1件は「substep ごとに積分してしまう」バグを
そのまま踏むテストです。

## 学び

ブラウザは方策のデバッグ環境として不適切でした。`rollout_check.py`（実 mjlab env で
ONNX を回して yaw を測る）を先に書いていれば、映像を見て悩む前に数字で切り分けられました。
